  ...
  const handlePayment = async (status) => {
    ...
    try {
      const response = await fetch(
        buildApiUrl(`${API_CONFIG.PAYMENT.BASE || "/payment"}/dummy-complete`),
        {
          method: "POST",
          headers: {
      ...
      if (data.success) {
        ...
        navigate(
          `/payment/callback?merchantTransactionId=${merchantTransactionId}`
        );
      } else {
    ...
    <div className="dummy-payment-wrapper">
      <div className="dummy-payment-container">
        ...
        <div className="payment-footer">
          <p>Protected by RealEstate Security</p>
        </div>
      </div>
    </div>
  );
