...
const AnalyticsOverview = () => {
  ...
  const fetchAnalytics = async (isRefresh = false) => {
    try {
      ...
      const bookingsResponse = await fetch(
        buildApiUrl(API_CONFIG.ADMIN.BOOKING_ANALYTICS),
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      const propsArray = propertiesData.data?.properties || [];

      // totalProperties: prefer explicit backend value, then pagination, then derive from array
      const totalPropertiesFromResponse =
        propertiesData.data?.totalProperties ||
        propertiesData.data?.pagination?.totalProperties ||
        propsArray.length;

      // statusBreakdown: prefer backend value, otherwise compute from the current page (best-effort)
      const statusBreakdownFromResponse =
        propertiesData.data?.statusBreakdown ||
        propsArray.reduce((acc, p) => {
          const s = (p.status || "unknown").toString();
          acc[s] = (acc[s] || 0) + 1;
          return acc;
        }, {});

      // set analytics using computed/fallback values
      setAnalytics({
        totalUsers: usersData.data?.totalUsers || 0,
        totalProperties: totalPropertiesFromResponse || 0,
        pendingProperties: statusBreakdownFromResponse.pending || 0,
        approvedProperties: statusBreakdownFromResponse.approved || 0,
        publishedProperties: statusBreakdownFromResponse.published || 0,
        rejectedProperties: statusBreakdownFromResponse.rejected || 0,
      });

      setBookingAnalytics({
        totalBookings: bookingsData.data?.totalBookings || 0,
        bookingsByStatus: bookingsData.data?.bookingsByStatus || {
          pending: 0,
          approved: 0,
          cancelled: 0,
        },
        bookingsByUserRole: bookingsData.data?.bookingsByUserRole || {
          user: 0,
          owner: 0,
        },
      ...
      // keep local copies used by UI helpers
  ...
  const getPendingActions = () => {
    ...
    const unverifiedOwners = properties.filter(
      (p) => p.owner && !p.owner.verified
    );
  ...
  const pendingPercentage = Math.round(
    (analytics.pendingProperties / totalProperties) * 100
  );
  const publishedPercentage = Math.round(
    (analytics.publishedProperties / totalProperties) * 100
  );
  ...
  const verifiedPercentage =
    analytics.totalUsers > 0
      ? Math.round((verifiedCount / analytics.totalUsers) * 100)
      : 0;

  const totalBookings = bookingAnalytics.totalBookings || 1;
  const approvedBookingsPercentage = Math.round(
    (bookingAnalytics.bookingsByStatus.approved / totalBookings) * 100
  );
  const pendingBookingsPercentage = Math.round(
    (bookingAnalytics.bookingsByStatus.pending / totalBookings) * 100
  );
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} sm={6} lg={3}>
          <StatCard
            ...
            color={theme.palette.info.main}
            subtitle={
              bookingAnalytics.topProperties[0]?.title?.substring(0, 25) +
                "..." || "No data available"
            }
          />
        </Grid>
              ...
              {bookingAnalytics.topProperties.length > 0 ? (
                <TableContainer>
                  <Table size="small">
                    ...
                    <TableBody>
                      {bookingAnalytics.topProperties
                        .slice(0, 5)
                        .map((property, index) => (
                          <TableRow
                ...
                <Stack spacing={3}>
                  {propertyTypes.map(([type, count]) => {
                    const percentage = Math.round(
                      (count / analytics.totalProperties) * 100
                    );
                    return (
                      <Box key={type}>
                        <Box
